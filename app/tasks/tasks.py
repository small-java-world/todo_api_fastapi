...
def process_task_notification(self, task_id: int, notification_type: str, data: Dict[str, Any]):
    """タスク通知の非同期処理"""
    try:
        db = SessionLocal()
        try:
    ...
    try:
        ...
        try:
            ...
            if report_type == "hierarchical_summary":
                logger.info(f"Generating hierarchical summary for {len(task_ids)} tasks")
                # 階層構造サマリーの生成
...
def cleanup_old_data(self, days_old: int = 30):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Cleanup completed: {deleted_history} history records, {deleted_comments} comments deleted")
            return {
                "status": "success", 
        ...
        try:
            ...
            for task in all_tasks:
                ...
                if task.parent_id:
                    ...
                    if not parent:
                        integrity_issues.append(f"Task {task.id} has invalid parent_id {task.parent_id}")
                    elif not task_service.hierarchical_id_service.validate_parent_child_relationship(parent, task.type):
                        integrity_issues.append(f"Task {task.id} has invalid parent-child relationship")
                
                # 階層IDの形式チェック
                if not task.hierarchical_id:
                    integrity_issues.append(f"Task {task.id} has no hierarchical_id")
                elif task.type == TaskType.requirement and not task.hierarchical_id.startswith("REQ-"):
                    integrity_issues.append(f"Task {task.id} has invalid hierarchical_id format")
                elif task.type == TaskType.task and ".TSK-" not in task.hierarchical_id:
                    integrity_issues.append(f"Task {task.id} has invalid hierarchical_id format")
                elif task.type == TaskType.subtask and ".SUB-" not in task.hierarchical_id:
                    integrity_issues.append(f"Task {task.id} has invalid hierarchical_id format")
            
            if integrity_issues:
                logger.warning(f"Hierarchical integrity issues found: {integrity_issues}")
