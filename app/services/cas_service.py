...
class CASService:
    ...
    def store_artifact(self, content: bytes, media_type: str, source_task_hid: Optional[str] = None, 
        ...
        # 既に存在するかチェック
        existing_artifact = self.db.query(Artifact).filter(Artifact.sha256 == sha256_hash).first()
    ...
    def get_artifact_info(self, sha256_hash: str) -> Optional[Dict[str, Any]]:
        """アーティファクトの情報を取得"""
        artifact = self.db.query(Artifact).filter(Artifact.sha256 == sha256_hash).first()
        if not artifact:
            return None
    ...
    def link_artifact_to_task(self, task_hid: str, sha256_hash: str, role: str) -> bool:
        ...
        # 既存のリンクをチェック
        existing_link = self.db.query(TaskArtifactLink).filter(
        ...
        if existing_link:
            logger.info(f"Link already exists for task {task_hid} and artifact {sha256_hash} with role {role}")
            return True
        ...
        logger.info(f"Linked artifact {sha256_hash} to task {task_hid} with role {role}")
        return True
    
    def get_task_artifacts(self, task_hid: str, role: Optional[str] = None) -> list:
        """タスクに関連するアーティファクトを取得"""
        query = self.db.query(TaskArtifactLink).filter(TaskArtifactLink.task_hid == task_hid)
    ...
    def delete_artifact(self, sha256_hash: str) -> bool:
        ...
        # リンクを削除
        self.db.query(TaskArtifactLink).filter(TaskArtifactLink.artifact_id == artifact.id).delete()
