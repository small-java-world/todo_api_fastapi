...
class GitService:
    ...
    def get_task_path(self, hierarchical_id: str) -> Path:
        ...
        if len(parts) == 1:  # REQ-001
            ...
            return self.git_root / "requirements" / parts[0] / "tasks" / parts[1] / "subtasks" / parts[2]
        else:
    ...
    def get_tests_path(self, hierarchical_id: str) -> Path:
        ...
        if ".TSK-" in hierarchical_id:
            ...
            raise ValueError(f"Tests path only available for tasks, not: {hierarchical_id}")
    
    def create_outline_file(self, hierarchical_id: str, outline_data: Dict[str, Any]) -> bool:
        """アウトラインJSONファイルを作成"""
        try:
            ...
            logger.error(f"Failed to create outline file for {hierarchical_id}: {str(e)}")
            return False
    ...
    def commit_changes(self, message: str) -> bool:
        ...
        try:
            ...
            # コミット
            subprocess.run(["git", "commit", "-m", message], cwd=self.git_root, check=True)
