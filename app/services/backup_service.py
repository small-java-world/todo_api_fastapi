...
class BackupService:
            ...
            if backup_dir.is_dir():
                ...
                if metadata_path.exists():
                    try:
                        ...
                        logger.warning(f"Failed to read metadata for {backup_dir.name}: {str(e)}")
                        backups.append({
                            "backup_name": backup_dir.name,
                            "created_at": None,
                            "backup_type": "unknown",
    ...
    def _restore_from_sql_dump(self, backup_dir: Path):
        ...
        if settings.database_url.startswith("postgresql://"):
            subprocess.run([
                "psql", settings.database_url
            ], stdin=open(dump_path, 'r'), check=True)
        # MySQLの場合
        elif settings.database_url.startswith("mysql://"):
            subprocess.run([
    ...
    def _create_sqlalchemy_dump(self, dump_path: Path):
        ...
        with open(dump_path, 'w', encoding='utf-8') as f:
            # テーブル構造をダンプ
            result = self.db.execute(text("SELECT sql FROM sqlite_master WHERE type='table'"))
            for row in result:
                f.write(f"{row[0]};\n")
            ...
            for row in result:
                ...
                if table_name != 'sqlite_sequence':
                    ...
                    for data_row in result:
                        values = ', '.join(f"'{str(v)}'" if v is not None else 'NULL' for v in data_row)
            ...
            for statement in sql_statements:
                ...
                if statement:
                    try:
                        ...
                        logger.warning(f"Failed to execute SQL: {statement[:100]}... Error: {str(e)}")
