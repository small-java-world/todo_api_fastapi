...
from app.schemas.task import (
    TaskCreate, TaskUpdate, StatusTransition, CommentCreate, Comment, TaskHistory
)
...
class TaskService:
    ...
    def create_task(self, task: TaskCreate) -> Task:
        ...
        # 階層IDを生成
        hierarchical_id = self.hierarchical_id_service.generate_hierarchical_id(parent, task.type)
    ...
    # 階層ナビゲーション用メソッド
    def get_requirements_summary(self, offset: int, limit: int, q: Optional[str], status: Optional[str]) -> List[Task]:
        """要件一覧（軽量）"""
    ...
    def search_tasks(self, type: Optional[str], status: Optional[str], parent_id: Optional[int], 
                    q: Optional[str], sort: str, order: str, offset: int, limit: int) -> List[Task]:
        """タスク検索・フィルタ"""
    ...
    def transition_status(self, task_id: int, transition: StatusTransition) -> Task:
        ...
        if not self._is_valid_transition(task.status, transition.to_status):
            raise ValueError(f"Invalid transition from {task.status} to {transition.to_status}")
        
        # 履歴を記録
        self._add_history(task_id, "status_change", task.status, transition.to_status, transition.reason)
        ...
        # TDDフックを実行
        try:
            self.tdd_hook_service.handle_status_transition(task, old_status, transition.to_status, transition.reason)
    ...
    def _add_history(self, task_id: int, event_type: str, from_status: Optional[str], 
                    to_status: Optional[str], note: Optional[str]):
        """履歴を追加"""
    ...
    def get_comments(self, task_id: int, offset: int, limit: int) -> List[Comment]:
        """コメント一覧"""
        comments = self.db.query(CommentModel).filter(
    ...
    def get_history(self, task_id: int, offset: int, limit: int) -> List[TaskHistory]:
        """履歴取得"""
        history_records = self.db.query(TaskHistoryModel).filter(
            TaskHistoryModel.task_id == task_id
        ).order_by(desc(TaskHistoryModel.created_at)).offset(offset).limit(limit).all()
    ...
    def get_task_by_hierarchical_id(self, hierarchical_id: str) -> Optional[Task]:
        """階層IDでタスクを取得"""
        return self.db.query(Task).filter(Task.hierarchical_id == hierarchical_id).first()
