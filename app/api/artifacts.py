...
from app.schemas.artifact import (
    ArtifactCreate, ArtifactInfo, TaskArtifactLinkCreate, TaskArtifactLink,
    ArtifactRetrieve, TaskSummaryCreate, TaskSummary, OutlineCard
)
...
def store_artifact(artifact: ArtifactCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not artifact_info:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve artifact info")
...
def get_artifact_content(sha256_hash: str, db: Session = Depends(get_db)):
    ...
    artifact_info = cas_service.get_artifact_info(sha256_hash)
    media_type = artifact_info["media_type"] if artifact_info else "application/octet-stream"
...
@router.post("/{sha256_hash}/link", response_model=TaskArtifactLink, status_code=status.HTTP_201_CREATED)
def link_artifact_to_task(
    sha256_hash: str,
    link_data: TaskArtifactLinkCreate,
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to link artifact to task")
...
# タスクサマリー管理API
@summaries_router.post("/", response_model=TaskSummary, status_code=status.HTTP_201_CREATED)
def create_task_summary(summary: TaskSummaryCreate, db: Session = Depends(get_db)):
    ...
    # 既存のサマリーをチェック
    existing = db.query(TaskSummaryModel).filter(TaskSummaryModel.task_hid == summary.task_hid).first()
    if existing:
        # 更新
...
def get_task_summary(task_hid: str, db: Session = Depends(get_db)):
    ...
    summary = db.query(TaskSummaryModel).filter(TaskSummaryModel.task_hid == task_hid).first()
    if not summary:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task summary not found")
...
def get_task_outline(task_hid: str, db: Session = Depends(get_db)):
    ...
    outline = OutlineCard(
        ...
        title=task.title,
        summary=summary.summary_140 if summary else task.description[:140] if task.description else "",
