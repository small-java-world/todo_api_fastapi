...
def create_git_outline(hierarchical_id: str, outline_data: Dict[str, Any]):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create outline file")
...
def get_git_outline(hierarchical_id: str):
    ...
    try:
        ...
        if outline_data is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Outline file not found")
...
def create_git_spec(hierarchical_id: str, content: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create spec file")
...
def get_git_spec(hierarchical_id: str):
    ...
    try:
        ...
        if content is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Spec file not found")
...
def initialize_git_repo():
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to initialize git repository")
...
def commit_git_changes(message: str = "Auto commit"):
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to commit changes")
